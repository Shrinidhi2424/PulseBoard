...
export const Column: React.FC<ColumnProps> = ({
  ...
  return (
    <section
      ...
      className="flex flex-col w-80 shrink-0 rounded-2xl bg-slate-900/60 border border-slate-800/80 p-4 max-h-[calc(100vh-10rem)] backdrop-blur-xs"
    >
      <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-800/60">
        ...
        {onAddTask && (
          <button
            ...
            className="p-1 rounded-lg text-slate-400 hover:text-slate-100 hover:bg-slate-800 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          >
            <svg
              className="w-4 h-4"
              fill="none"
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-1 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
        {tasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-32 border-2 border-dashed border-slate-800/60 rounded-xl text-xs text-slate-500">
            No tasks yet
          </div>
        ) : (
          tasks.map((task) => (
